...
for userY, userYRatings in songData3.items():
    users = songData3[userY]
    SIMS = similarity(userXRatings,users)
    if (userY != userX):
        userDistances.append((userY, round(SIMS.minkowski(2),2)))
...
userSortedDistances = []
# <<<<< (2) YOUR CODE HERE >>>>>
userSortedDistances = userDistances 
userSortedDistances = sorted (userSortedDistances, key=itemgetter(1), reverse=False)
...
userXSortedRecos = sorted(userXSortedRecos, key = itemgetter(1), reverse = True)
